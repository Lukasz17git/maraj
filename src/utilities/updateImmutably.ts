...
const immutableImplementation: ImmutableImplementation = (state, updates, addNonExistentPropsAndIndexes) => {
   ...
   for (const [pathSeparatedByDots, newValueToUpdate] of Object.entries(updates)) {
      ...
      for (let indexKey = 0; indexKey < pathKeysToReachLastKey.length; indexKey++) {
         ...
         /* checking if the currentParent[currentKey] is undefined */
         const isUndefined = valueInCurrentKey === undefined

         if (!isObjectOrArray && !isUndefined) throw new Error(`Property "${currentKey}" in "${pathKeysToReachLastKey.slice(0, indexKey).join('.')}" is not an object`)
         if (isUndefined && !addNonExistentPropsAndIndexes) throw new Error(`"${currentKey}" doesnt exist in "${pathKeysToReachLastKey.slice(0, indexKey + 1).join('.')}"`)
...
type ImmutableUpdate = <T extends object>(state: T, updates: DotPathUpdateObject<T>) => T
 ...
 */
export const immutableUpdate: ImmutableUpdate = (state, updates) => immutableImplementation(state, updates, false)

type Expanded<T, U> = T
type AutoExpandableImmutableUpdate = <T extends object, U extends DotPathUpdateObject<T>>(state: T, updates: U) => Expanded<T, U>
 ...
 * @returns New Updated Object.
 */
export const autoExpandableImmutableUpdate: AutoExpandableImmutableUpdate = (state, updates) => immutableImplementation(state, updates, true)